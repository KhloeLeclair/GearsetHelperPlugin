...
internal class Exporter : IDisposable {
	...
	private ExportResponse Task_ExportTeamcraft(EquipmentSet gearset) {
		...
		try {
			...
			Plugin.PluginLog.Error($"An error occurred while exporting gearset to Teamcraft.\nDetails: {ex}");
	...
	private async Task<EtroLoginResponse> Task_EtroLogin(string username, string password) {
		...
		try {
			...
			if (response.IsSuccessStatusCode) {
				...
				Plugin.PluginLog.Information($"Success. Response: {value}");
				...
				string? value = await response.Content.ReadAsStringAsync();
				Plugin.PluginLog.Error($"Failure. Error: {response.StatusCode}\nDetails:{value}");
			...
			Plugin.PluginLog.Error($"An error occurred while logging in to Etro.\nDetails: {ex}");
	...
	private async Task<ExportResponse> Task_ExportEtro(EquipmentSet gearset) {
		...
		try {
			...
			Plugin.PluginLog.Information("Exporting gearset to Etro.");
			...
			foreach (var item in gearset.Items) {
				...
				if (slot == 12) {
					...
					Plugin.PluginLog.Information($"Unknown Slot for Item: {data.Name} -- Slot: {slot}");
					continue;
				}
				...
				if (obj.ContainsKey(mappedSlot)) {
					Plugin.PluginLog.Information($"Duplicate item slot usage for Item: {data.Name} -- Slot: {slot} = {mappedSlot}");
					continue;
				}
			...
			Plugin.PluginLog.Information($"Result:\n{obj.ToString(Formatting.None)}");
			...
			Plugin.PluginLog.Error($"An error occurred while exporting gearset to Etro.\nDetails: {ex}");
			result.Error = "An error occurred. See the log for details.";
		}
	...
	private async Task<ExportResponse> Task_ExportAriyala(EquipmentSet gearset) {
		...
		try {
			Plugin.PluginLog.Information("Exporting gearset to Ariyala.");
			...
			foreach (var rawItem in gearset.Items) {
				...
				if (slot == 12) {
					...
					Plugin.PluginLog.Information($"Unknown Slot for Item: {item.Name} -- Slot: {slot}");
					continue;
				}
				...
				foreach (var rawMateria in rawItem.Melds) {
					...
					if (!ARIYALA_STAT_MAP.TryGetValue(stat, out string? mappedStat)) {
						Plugin.PluginLog.Information($"Unknown Stat for Materia: {materia.Item[rawMateria.Grade]?.Value?.Name} -- Stat: {stat}");
						continue;
					}
			...
			if (response.IsSuccessStatusCode) {
				string? value = await response.Content.ReadAsStringAsync();
				Plugin.PluginLog.Information($"Success. ID: {value}");
			...
			/* Do nothing~ */
			Plugin.PluginLog.Error($"An error occurred while exporting gearset to Ariyala.\nDetails: {ex}");
